...
const JobCardItem = ({ job, userId }: JobCardItemProps) => {
  ...
  return (
    <motion.div layout className="w-full sm:max-w-[500px] md:max-w-[600px]">
      ...
      <Card>
        <div className="w-full h-full p-4 flex flex-col items-start justify-start gap-y-4">
          ...
          {/* company details */}
          <Box className="flex-wrap justify-start gap-1 max-w-full">
          ...
          {job.hourlyRate && (
            <div className="text-xs text-muted-foreground flex items-center">
              ...
              {`NLe ${formattedString(job.hourlyRate)} /hr`}
            </div>
          )}
          ...
          {job.short_description && (
            <CardDescription className="text-xs w-full break-words">
          ...
          {job.tags.length > 0 && (
           <Box className="flex flex-wrap justify-start gap-1 max-w-full">
           {job.tags.slice(0, 6).map((tag, i) => (
             <p
               key={i}
               className="bg-gray-100 text-xs rounded-md px-2 py-[2px] font-semibold text-neutral-500 truncate"
             >
               {tag}
             </p>
           ))}
